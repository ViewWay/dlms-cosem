...
class TestHdlcParameter:
    ...
    def test_create_window_size_parameter(self):
        ...
        assert param.param_type == HdlcParameterType.WINDOW_SIZE_TX
    ...
    def test_window_size_validation_passes(self):
        ...
        param = HdlcParameter(HdlcParameterType.WINDOW_SIZE_TX, 5)
    ...
    def test_window_size_validation_fails_high(self):
        ...
        param = HdlcParameter(HdlcParameterType.WINDOW_SIZE_TX, 10)
        with pytest.raises(ValueError, match="out of range"):
            param.validate()
    ...
    def test_encode_window_size_parameter(self):
        ...
        assert encoded == b"\x07\x01\x03"  # type=0x07, length=1, value=3

    def test_encode_max_info_length_parameter(self):
        """Test encoding a max info length parameter."""
        param = HdlcParameter(HdlcParameterType.MAX_INFORMATION_FIELD_LENGTH_TX, 512)
        encoded = param.to_bytes()
        assert encoded == b"\x05\x02\x02\x00"  # type=0x05, length=2, value=512
    ...
    def test_decode_fails_with_insufficient_value_data(self):
        ...
        data = b"\x05\x02\x00"  # type=0x05, length=2, but only 1 byte of value
    ...
    def test_bytes_magic_method(self):
        ...
        param = HdlcParameter(HdlcParameterType.WINDOW_SIZE_TX, 3)
        assert bytes(param) == b"\x07\x01\x03"
...
class TestHdlcParameterList:
    ...
    def test_get_returns_value_when_set(self):
        ...
        params = HdlcParameterList()
        params.set(HdlcParameterType.WINDOW_SIZE_TX, 5)
        assert params.get(HdlcParameterType.WINDOW_SIZE_TX) == 5

    def test_get_returns_default_when_not_set(self):
        """Test that get returns default when parameter is not set."""
        params = HdlcParameterList()
        assert params.get(HdlcParameterType.WINDOW_SIZE_TX, 99) == 99
    ...
    def test_get_returns_none_when_not_set_and_no_default(self):
        ...
        params = HdlcParameterList()
        assert params.get(HdlcParameterType.WINDOW_SIZE_TX) is None
    ...
    def test_encode_single_parameter(self):
        ...
        # Header: 0x81 0x80 len(6), then WINDOW_SIZE_TX(0x07)=3, WINDOW_SIZE_RX(0x08)=3
        assert encoded == b"\x81\x80\x06\x07\x01\x03\x08\x01\x03"
    ...
    def test_encode_multiple_parameters(self):
        ...
        params.set_max_info_length_tx(512)
        encoded = params.to_bytes()
        # Header: 0x81 0x80 len(10), then params sorted by type: 0x05, 0x07, 0x08
        assert encoded == b"\x81\x80\x0a\x05\x02\x02\x00\x07\x01\x03\x08\x01\x03"
    ...
    def test_decode_single_parameter(self):
        ...
        data = b"\x81\x80\x02\x07\x01\x03"
    ...
    def test_decode_multiple_parameters(self):
        ...
        data = b"\x81\x80\x08\x05\x02\x02\x00\x07\x01\x03\x08\x01\x03"
        params = HdlcParameterList.from_bytes(data)
        assert len(params) == 3
        assert params.window_size == 3
        assert params.max_info_length_tx == 512
    ...
    def test_contains_operator(self):
        ...
        params.set_window_size_tx(3)
        params.set_window_size_rx(3)
        assert HdlcParameterType.WINDOW_SIZE_TX in params
    ...
    def test_repr(self):
        ...
        assert "WINDOW_SIZE_TX=3" in repr_str
        assert "WINDOW_SIZE_RX=3" in repr_str
    ...
    def test_set_validates_parameter(self):
        ...
        params = HdlcParameterList()
        with pytest.raises(ValueError, match="out of range"):
            params.set_window_size_tx(10)
        with pytest.raises(ValueError, match="out of range"):
            params.set_window_size_rx(10)

    def test_merge_two_lists(self):
        """Test merging two parameter lists."""
        params1 = HdlcParameterList()
        params1.set_window_size_tx(3)
        params1.set_window_size_rx(3)
    ...
    def test_merge_overwrites_values(self):
        ...
        params1.set_window_size_rx(3)
        params2 = HdlcParameterList()
        params2.set_window_size_tx(5)
        params2.set_window_size_rx(5)
        merged = params1.merge(params2)
...
class TestNegotiateParameters:
    ...
    def test_negotiate_window_size_minimum(self):
        ...
        client.set_window_size_rx(5)
        server = HdlcParameterList()
        server.set_window_size_tx(3)
    ...
    def test_negotiate_all_parameters(self):
        ...
        negotiated = negotiate_parameters(client, server)
        assert negotiated.window_size == 3
        # Client TX(1024) matched with server RX(1024) -> min = 1024
...
class TestSetNormalResponseModeFrame:
    ...
    def test_snrm_with_parameters(self):
        ...
        # Information field should contain encoded parameters with header
        assert frame.information == b"\x81\x80\x0a\x05\x02\x02\x00\x07\x01\x03\x08\x01\x03"
    ...
    def test_snrm_frame_length_with_parameters(self):
        ...
        expected_length = (
            ...
            + 9  # parameters with header (0x81 0x80 len=6 + 2 params * 3 bytes each = 9 total)
...
class TestUnNumberedAcknowledgmentFrame:
    ...
    def test_ua_with_parameters(self):
        ...
        assert frame.information == b"\x81\x80\x06\x07\x01\x03\x08\x01\x03"
    ...
    def test_ua_with_both_parameters_and_payload(self):
        ...
        assert frame.information == b"\x81\x80\x06\x07\x01\x03\x08\x01\x03\xff\xff"
