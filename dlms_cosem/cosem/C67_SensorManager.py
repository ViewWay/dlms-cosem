...
class SensorManager:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: sensor_list (array)
        result += self._encode_array(self.sensor_list)
        # Attribute 4: sensor_status (array)
        result += self._encode_array(self.sensor_status)
        # Attribute 5: totals (array)
        result += self._encode_array(self.totals)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SensorManager':
        ...
        # Attribute 3: sensor_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sensor_list'] = cls._decode_array(value)
        # Attribute 4: sensor_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sensor_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: totals
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['totals'] = cls._decode_array(value)
        return cls(**kwargs)
