...
class Credit:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: current_credit_amount (float64)
        result += self._encode_float64(self.current_credit_amount)
        # Attribute 4: type (integer)
        result += self._encode_integer(self.type)
        # Attribute 5: priority (integer)
        result += self._encode_integer(self.priority)
        # Attribute 6: warning_threshold (float64)
        result += self._encode_float64(self.warning_threshold)
        # Attribute 7: limit (float64)
        result += self._encode_float64(self.limit)
        # Attribute 8: credit_configuration (integer)
        result += self._encode_integer(self.credit_configuration)
        # Attribute 9: status (integer)
        result += self._encode_integer(self.status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'Credit':
        ...
        # Attribute 3: current_credit_amount
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
        ...
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['type'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: priority
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['priority'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: warning_threshold
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['warning_threshold'] = cls._decode_float64(value) if value else 0.0
        # Attribute 7: limit
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['limit'] = cls._decode_float64(value) if value else 0.0
        # Attribute 8: credit_configuration
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['credit_configuration'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 9: status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
