...
class WirelessModeQChannel:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: channel_info (array)
        result += self._encode_array(self.channel_info)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'WirelessModeQChannel':
        ...
        # Attribute 3: channel_info
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['channel_info'] = cls._decode_array(value)
        return cls(**kwargs)
