...
class SFSKReportingSystemList:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: reporting_system_list (array)
        result += self._encode_array(self.reporting_system_list)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SFSKReportingSystemList':
        ...
        # Attribute 3: reporting_system_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['reporting_system_list'] = cls._decode_array(value)
        return cls(**kwargs)
