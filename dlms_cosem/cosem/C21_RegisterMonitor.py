...
class RegisterMonitor:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: thresholds (array)
        result += self._encode_array(self.thresholds)
        # Attribute 4: monitored_object (string)
        result += self._encode_string(self.monitored_object or '')
        # Attribute 5: monitored_attribute (integer)
        result += self._encode_integer(self.monitored_attribute)
        # Attribute 6: selected_object (string)
        result += self._encode_string(self.selected_object or '')
        # Attribute 7: selected_attribute (integer)
        result += self._encode_integer(self.selected_attribute)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'RegisterMonitor':
        ...
        # Attribute 3: thresholds
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['thresholds'] = cls._decode_array(value)
        # Attribute 4: monitored_object
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['monitored_object'] = value.decode('utf-8') if value else ''
        # Attribute 5: monitored_attribute
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['monitored_attribute'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: selected_object
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['selected_object'] = value.decode('utf-8') if value else ''
        # Attribute 7: selected_attribute
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['selected_attribute'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
