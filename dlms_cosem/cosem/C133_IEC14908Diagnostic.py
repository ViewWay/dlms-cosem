...
class IEC14908Diagnostic:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 7: timeouts (integer)
        result += self._encode_integer(self.timeouts)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC14908Diagnostic':
        ...
        # Attribute 7: timeouts
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['timeouts'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
