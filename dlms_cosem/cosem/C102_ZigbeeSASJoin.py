...
class ZigbeeSASJoin:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: join_control (integer)
        result += self._encode_integer(self.join_control)
        # Attribute 4: rejoin_interval (integer)
        result += self._encode_integer(self.rejoin_interval)
        # Attribute 5: max_rejoin_interval (integer)
        result += self._encode_integer(self.max_rejoin_interval)
        # Attribute 6: security_level (integer)
        result += self._encode_integer(self.security_level)
        # Attribute 7: network_key_enable (boolean)
        result += self._encode_boolean(self.network_key_enable)
        # Attribute 8: preconfigured_link_key (octet-string)
        result += self._encode_octet_string(self.preconfigured_link_key or b'')
        # Attribute 9: trust_center_address (octet-string)
        result += self._encode_octet_string(self.trust_center_address or b'')
        # Attribute 10: trust_center_master_key (octet-string)
        result += self._encode_octet_string(self.trust_center_master_key or b'')
        # Attribute 11: active_network_key_seq_number (integer)
        result += self._encode_integer(self.active_network_key_seq_number)
        # Attribute 12: link_key (octet-string)
        result += self._encode_octet_string(self.link_key or b'')
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ZigbeeSASJoin':
        ...
        # Attribute 3: join_control
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['join_control'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: rejoin_interval
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rejoin_interval'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: max_rejoin_interval
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['max_rejoin_interval'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: security_level
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['security_level'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: network_key_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_key_enable'] = bool(value[0]) if value else False
        # Attribute 8: preconfigured_link_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['preconfigured_link_key'] = value
        # Attribute 9: trust_center_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['trust_center_address'] = value
        # Attribute 10: trust_center_master_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['trust_center_master_key'] = value
        # Attribute 11: active_network_key_seq_number
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_network_key_seq_number'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 12: link_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
