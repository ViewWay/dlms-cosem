...
class StatusMapping:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mappings (array)
        result += self._encode_array(self.mappings)
        # Attribute 4: default_external_status (integer)
        result += self._encode_integer(self.default_external_status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'StatusMapping':
        ...
        # Attribute 3: mappings
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mappings'] = cls._decode_array(value)
        # Attribute 4: default_external_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['default_external_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
