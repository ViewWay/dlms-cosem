...
class MBusMasterPortSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: comm_speed (integer)
        result += self._encode_integer(self.comm_speed)
        # Attribute 4: rec_timeout (integer)
        result += self._encode_integer(self.rec_timeout)
        # Attribute 5: send_timeout (integer)
        result += self._encode_integer(self.send_timeout)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'MBusMasterPortSetup':
        ...
        # Attribute 3: comm_speed
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['comm_speed'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: rec_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rec_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: send_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['send_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
