...
class MACAddressSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mac_address (integer)
        result += self._encode_octet_string(bytes(self.mac_address))
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'MACAddressSetup':
        ...
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_address'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
