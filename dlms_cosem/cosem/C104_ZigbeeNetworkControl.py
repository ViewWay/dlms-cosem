...
class ZigbeeNetworkControl:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: network_mode (integer)
        result += self._encode_integer(self.network_mode)
        # Attribute 4: pan_id (integer)
        result += self._encode_integer(self.pan_id)
        # Attribute 5: extended_pan_id (octet-string)
        result += self._encode_octet_string(self.extended_pan_id or b'')
        # Attribute 6: channel (integer)
        result += self._encode_integer(self.channel)
        # Attribute 7: permit_duration (integer)
        result += self._encode_integer(self.permit_duration)
        # Attribute 8: device_timeout (integer)
        result += self._encode_integer(self.device_timeout)
        # Attribute 9: router_capacity (integer)
        result += self._encode_integer(self.router_capacity)
        # Attribute 10: end_device_capacity (integer)
        result += self._encode_integer(self.end_device_capacity)
        # Attribute 11: trust_center_address (octet-string)
        result += self._encode_octet_string(self.trust_center_address or b'')
        # Attribute 12: trust_center_master_key (octet-string)
        result += self._encode_octet_string(self.trust_center_master_key or b'')
        # Attribute 13: active_network_key_seq_number (integer)
        result += self._encode_integer(self.active_network_key_seq_number)
        # Attribute 14: network_key (octet-string)
        result += self._encode_octet_string(self.network_key or b'')
        # Attribute 15: link_key (octet-string)
        result += self._encode_octet_string(self.link_key or b'')
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ZigbeeNetworkControl':
        ...
        # Attribute 5: extended_pan_id
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['extended_pan_id'] = value
        # Attribute 6: channel
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['channel'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: permit_duration
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['permit_duration'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: device_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['device_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 9: router_capacity
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['router_capacity'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: end_device_capacity
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['end_device_capacity'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: trust_center_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['trust_center_address'] = value
        # Attribute 12: trust_center_master_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['trust_center_master_key'] = value
        # Attribute 13: active_network_key_seq_number
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_network_key_seq_number'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 14: network_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_key'] = value
        # Attribute 15: link_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['link_key'] = value
        return cls(**kwargs)
