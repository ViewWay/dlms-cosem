...
class LoRaWANSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: lorawan_device_eui (octet-string)
        result += self._encode_octet_string(self.lorawan_device_eui or b'')
        # Attribute 4: lorawan_app_eui (octet-string)
        result += self._encode_octet_string(self.lorawan_app_eui or b'')
        # Attribute 5: lorawan_app_key (octet-string)
        result += self._encode_octet_string(self.lorawan_app_key or b'')
        # Attribute 6: lorawan_nwk_s_key (octet-string)
        result += self._encode_octet_string(self.lorawan_nwk_s_key or b'')
        # Attribute 7: lorawan_app_s_key (octet-string)
        result += self._encode_octet_string(self.lorawan_app_s_key or b'')
        # Attribute 8: lorawan_dev_addr (octet-string)
        result += self._encode_octet_string(self.lorawan_dev_addr or b'')
        # Attribute 9: lorawan_uplink_counter (integer)
        result += self._encode_integer(self.lorawan_uplink_counter)
        # Attribute 10: lorawan_downlink_counter (integer)
        result += self._encode_integer(self.lorawan_downlink_counter)
        # Attribute 11: lorawan_adr (boolean)
        result += self._encode_boolean(self.lorawan_adr)
        # Attribute 12: lorawan_rx2_data_rate (integer)
        result += self._encode_integer(self.lorawan_rx2_data_rate)
        # Attribute 13: lorawan_rx1_delay (integer)
        result += self._encode_integer(self.lorawan_rx1_delay)
        # Attribute 14: lorawan_rx2_frequency (integer)
        result += self._encode_integer(self.lorawan_rx2_frequency)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'LoRaWANSetup':
        ...
        # Attribute 3: lorawan_device_eui
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_device_eui'] = value
        # Attribute 4: lorawan_app_eui
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_app_eui'] = value
        # Attribute 5: lorawan_app_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_app_key'] = value
        # Attribute 6: lorawan_nwk_s_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_nwk_s_key'] = value
        # Attribute 7: lorawan_app_s_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_app_s_key'] = value
        # Attribute 8: lorawan_dev_addr
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_dev_addr'] = value
        # Attribute 9: lorawan_uplink_counter
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_uplink_counter'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: lorawan_downlink_counter
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_downlink_counter'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: lorawan_adr
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_adr'] = bool(value[0]) if value else False
        # Attribute 12: lorawan_rx2_data_rate
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_rx2_data_rate'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 13: lorawan_rx1_delay
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_rx1_delay'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 14: lorawan_rx2_frequency
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_rx2_frequency'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
