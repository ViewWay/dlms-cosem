...
class SMTPSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: server_address (string)
        result += self._encode_string(self.server_address or '')
        # Attribute 4: server_port (integer)
        result += self._encode_integer(self.server_port)
        ...
        # Attribute 7: use_tls (boolean)
        result += self._encode_boolean(self.use_tls)
        # Attribute 8: sender_address (string)
        result += self._encode_string(self.sender_address or '')
        # Attribute 9: recipient_list (array)
        result += self._encode_array(self.recipient_list)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SMTPSetup':
        ...
        # Attribute 3: server_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['server_address'] = value.decode('utf-8') if value else ''
        # Attribute 4: server_port
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['server_port'] = int.from_bytes(value, 'big', signed=True) if value else 0
        ...
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['use_tls'] = bool(value[0]) if value else False
        # Attribute 8: sender_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sender_address'] = value.decode('utf-8') if value else ''
        # Attribute 9: recipient_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['recipient_list'] = cls._decode_array(value)
        return cls(**kwargs)
