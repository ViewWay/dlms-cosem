...
class PRIMEPhysicalCounters:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: phy_tx_drop (integer)
        result += self._encode_integer(self.phy_tx_drop)
        # Attribute 4: phy_rx_total (integer)
        result += self._encode_integer(self.phy_rx_total)
        # Attribute 5: phy_rx_crc_error (integer)
        result += self._encode_integer(self.phy_rx_crc_error)
        # Attribute 6: phy_tx_total (integer)
        result += self._encode_integer(self.phy_tx_total)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'PRIMEPhysicalCounters':
        ...
        # Attribute 3: phy_tx_drop
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_tx_drop'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: phy_rx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_rx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: phy_rx_crc_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_rx_crc_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: phy_tx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
