...
class AssociationSN:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: object_list (array)
        result += self._encode_array(self.object_list)
        # Attribute 4: association_status (integer)
        result += self._encode_integer(self.association_status)
        # Attribute 5: security_setup_reference (string)
        result += self._encode_string(self.security_setup_reference or '')
        # Attribute 6: hls_auth_mechanism
        result += self._encode_octet_string(self._any_to_bytes(self.hls_auth_mechanism))
        # Attribute 7: server_system_title (octet-string)
        result += self._encode_octet_string(self.server_system_title or b'')
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'AssociationSN':
        ...
        # Attribute 3: object_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['object_list'] = cls._decode_array(value)
        # Attribute 4: association_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['association_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: security_setup_reference
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['security_setup_reference'] = value.decode('utf-8') if value else ''
        # Attribute 6: hls_auth_mechanism
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['hls_auth_mechanism'] = value
        # Attribute 7: server_system_title
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['server_system_title'] = value
        return cls(**kwargs)
