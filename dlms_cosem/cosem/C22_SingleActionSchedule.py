...
class SingleActionSchedule:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: executed_script (string)
        result += self._encode_string(self.executed_script or '')
        # Attribute 4: schedule_type (integer)
        result += self._encode_integer(self.schedule_type)
        # Attribute 5: execution_time (octet-string)
        result += self._encode_octet_string(self.execution_time or b'')
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SingleActionSchedule':
        ...
        # Attribute 3: executed_script
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['executed_script'] = value.decode('utf-8') if value else ''
        # Attribute 4: schedule_type
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['schedule_type'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: execution_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['execution_time'] = value
        return cls(**kwargs)
