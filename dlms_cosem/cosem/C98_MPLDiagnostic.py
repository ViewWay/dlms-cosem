...
class MPLDiagnostic:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mpl_domain_id (octet-string)
        result += self._encode_octet_string(self.mpl_domain_id or b'')
        # Attribute 4: mpl_seed_set_version (integer)
        result += self._encode_integer(self.mpl_seed_set_version)
        # Attribute 5: mpl_trickle_timer_expirations (integer)
        result += self._encode_integer(self.mpl_trickle_timer_expirations)
        # Attribute 6: mpl_messages_sent (integer)
        result += self._encode_integer(self.mpl_messages_sent)
        # Attribute 7: mpl_messages_received (integer)
        result += self._encode_integer(self.mpl_messages_received)
        # Attribute 8: mpl_messages_forwarded (integer)
        result += self._encode_integer(self.mpl_messages_forwarded)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'MPLDiagnostic':
        ...
        # Attribute 3: mpl_domain_id
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_domain_id'] = value
        # Attribute 4: mpl_seed_set_version
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_seed_set_version'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: mpl_trickle_timer_expirations
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_trickle_timer_expirations'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: mpl_messages_sent
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_messages_sent'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: mpl_messages_received
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_messages_received'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: mpl_messages_forwarded
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mpl_messages_forwarded'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
