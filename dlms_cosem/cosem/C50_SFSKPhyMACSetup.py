...
class SFSKPhyMACSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 4: phy_list (array)
        result += self._encode_array(self.phy_list)
        # Attribute 5: tx_level (integer)
        result += self._encode_integer(self.tx_level)
        # Attribute 6: rx_level (integer)
        result += self._encode_integer(self.rx_level)
        # Attribute 7: frequency_band (integer)
        result += self._encode_integer(self.frequency_band)
        # Attribute 8: data_rate (integer)
        result += self._encode_integer(self.data_rate)
        # Attribute 9: tx_frequency (integer)
        result += self._encode_integer(self.tx_frequency)
        # Attribute 10: rx_frequency (integer)
        result += self._encode_integer(self.rx_frequency)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SFSKPhyMACSetup':
        ...
        # Attribute 4: phy_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_list'] = cls._decode_array(value)
        # Attribute 5: tx_level
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tx_level'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: rx_level
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rx_level'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: frequency_band
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['frequency_band'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: data_rate
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['data_rate'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 9: tx_frequency
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tx_frequency'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: rx_frequency
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rx_frequency'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
