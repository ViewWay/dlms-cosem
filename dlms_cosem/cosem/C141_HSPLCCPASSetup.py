...
class HSPLCCPASSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: cpas_enable (boolean)
        result += self._encode_boolean(self.cpas_enable)
        # Attribute 4: cpas_mtu (integer)
        result += self._encode_integer(self.cpas_mtu)
        # Attribute 5: cpas_fragmentation_timeout (integer)
        result += self._encode_integer(self.cpas_fragmentation_timeout)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'HSPLCCPASSetup':
        ...
        # Attribute 3: cpas_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['cpas_enable'] = bool(value[0]) if value else False
        # Attribute 4: cpas_mtu
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['cpas_mtu'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: cpas_fragmentation_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['cpas_fragmentation_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
