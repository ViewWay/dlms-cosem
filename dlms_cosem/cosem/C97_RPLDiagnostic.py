...
class RPLDiagnostic:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: parent_address (octet-string)
        result += self._encode_octet_string(self.parent_address or b'')
        # Attribute 4: parent_rank (integer)
        result += self._encode_integer(self.parent_rank)
        # Attribute 5: parent_link_metric (integer)
        result += self._encode_integer(self.parent_link_metric)
        # Attribute 6: parent_link_metric_type (integer)
        result += self._encode_integer(self.parent_link_metric_type)
        # Attribute 7: parent_switches (integer)
        result += self._encode_integer(self.parent_switches)
        # Attribute 8: children_addresses (array)
        result += self._encode_array(self.children_addresses)
        # Attribute 9: children_ranks (array)
        result += self._encode_array(self.children_ranks)
        # Attribute 10: dao_messages_sent (integer)
        result += self._encode_integer(self.dao_messages_sent)
        # Attribute 11: dao_messages_received (integer)
        result += self._encode_integer(self.dao_messages_received)
        # Attribute 12: dio_messages_sent (integer)
        result += self._encode_integer(self.dio_messages_sent)
        # Attribute 13: dio_messages_received (integer)
        result += self._encode_integer(self.dio_messages_received)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'RPLDiagnostic':
        ...
        # Attribute 3: parent_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parent_address'] = value
        # Attribute 4: parent_rank
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parent_rank'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: parent_link_metric
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parent_link_metric'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: parent_link_metric_type
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parent_link_metric_type'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: parent_switches
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parent_switches'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: children_addresses
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['children_addresses'] = cls._decode_array(value)
        # Attribute 9: children_ranks
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['children_ranks'] = cls._decode_array(value)
        # Attribute 10: dao_messages_sent
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['dao_messages_sent'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: dao_messages_received
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['dao_messages_received'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 12: dio_messages_sent
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['dio_messages_sent'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 13: dio_messages_received
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['dio_messages_received'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
