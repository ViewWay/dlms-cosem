...
class WiSUNDiagnostic:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: messages_sent (integer)
        result += self._encode_integer(self.messages_sent)
        # Attribute 4: messages_received (integer)
        result += self._encode_integer(self.messages_received)
        # Attribute 5: messages_failed (integer)
        result += self._encode_integer(self.messages_failed)
        # Attribute 6: messages_retransmitted (integer)
        result += self._encode_integer(self.messages_retransmitted)
        # Attribute 7: phy_tx_total (integer)
        result += self._encode_integer(self.phy_tx_total)
        # Attribute 8: phy_rx_total (integer)
        result += self._encode_integer(self.phy_rx_total)
        # Attribute 9: phy_tx_error (integer)
        result += self._encode_integer(self.phy_tx_error)
        # Attribute 10: phy_rx_error (integer)
        result += self._encode_integer(self.phy_rx_error)
        # Attribute 11: mac_tx_total (integer)
        result += self._encode_integer(self.mac_tx_total)
        # Attribute 12: mac_rx_total (integer)
        result += self._encode_integer(self.mac_rx_total)
        # Attribute 13: mac_tx_error (integer)
        result += self._encode_integer(self.mac_tx_error)
        # Attribute 14: mac_rx_error (integer)
        result += self._encode_integer(self.mac_rx_error)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'WiSUNDiagnostic':
        ...
        # Attribute 3: messages_sent
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['messages_sent'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: messages_received
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['messages_received'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: messages_failed
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['messages_failed'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: messages_retransmitted
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['messages_retransmitted'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: phy_tx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_tx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: phy_rx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_rx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 9: phy_tx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_tx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: phy_rx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_rx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: mac_tx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 12: mac_rx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 13: mac_tx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 14: mac_rx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
