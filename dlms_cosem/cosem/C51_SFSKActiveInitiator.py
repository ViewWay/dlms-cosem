...
class SFSKActiveInitiator:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: active_initiator (array)
        result += self._encode_array(self.active_initiator)
        # Attribute 4: active_initiator_count (integer)
        result += self._encode_integer(self.active_initiator_count)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SFSKActiveInitiator':
        ...
        # Attribute 3: active_initiator
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_initiator'] = cls._decode_array(value)
        # Attribute 4: active_initiator_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_initiator_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
