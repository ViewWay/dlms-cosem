...
class UtilityTables:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: table_cell_values (array)
        result += self._encode_array(self.table_cell_values)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'UtilityTables':
        ...
        # Attribute 3: table_cell_values
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['table_cell_values'] = cls._decode_array(value)
        return cls(**kwargs)
