...
class Arbitrator:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: controls (array)
        result += self._encode_array(self.controls)
        # Attribute 4: permissions (array)
        result += self._encode_array(self.permissions)
        # Attribute 5: arbitration_table (array)
        result += self._encode_array(self.arbitration_table)
        # Attribute 6: active_controls (array)
        result += self._encode_array(self.active_controls)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'Arbitrator':
        ...
        # Attribute 3: controls
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['controls'] = cls._decode_array(value)
        # Attribute 4: permissions
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['permissions'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: arbitration_table
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['arbitration_table'] = cls._decode_array(value)
        # Attribute 6: active_controls
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_controls'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
