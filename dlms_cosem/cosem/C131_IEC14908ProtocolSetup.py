...
class IEC14908ProtocolSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: protocol_mode (integer)
        result += self._encode_integer(self.protocol_mode)
        # Attribute 4: protocol_version (integer)
        result += self._encode_integer(self.protocol_version)
        # Attribute 5: protocol_parameters (array)
        result += self._encode_array(self.protocol_parameters)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC14908ProtocolSetup':
        ...
        # Attribute 3: protocol_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['protocol_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: protocol_version
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['protocol_version'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: protocol_parameters
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['protocol_parameters'] = cls._decode_array(value)
        return cls(**kwargs)
