...
class WiSUNSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: phy_operating_mode (integer)
        result += self._encode_integer(self.phy_operating_mode)
        # Attribute 4: network_mode (integer)
        result += self._encode_integer(self.network_mode)
        # Attribute 5: pan_id (integer)
        result += self._encode_integer(self.pan_id)
        # Attribute 6: routing_method (integer)
        result += self._encode_integer(self.routing_method)
        # Attribute 7: routing_method_parameters (array)
        result += self._encode_array(self.routing_method_parameters)
        # Attribute 8: phy_operating_mode_list (array)
        result += self._encode_array(self.phy_operating_mode_list)
        # Attribute 9: channel_function (integer)
        result += self._encode_integer(self.channel_function)
        # Attribute 10: channel_hopping_mode (integer)
        result += self._encode_integer(self.channel_hopping_mode)
        # Attribute 11: unicast_dwell_time (integer)
        result += self._encode_integer(self.unicast_dwell_time)
        # Attribute 12: broadcast_dwell_time (integer)
        result += self._encode_integer(self.broadcast_dwell_time)
        # Attribute 13: broadcast_interval (integer)
        result += self._encode_integer(self.broadcast_interval)
        # Attribute 14: broadcast_sequence_number (integer)
        result += self._encode_integer(self.broadcast_sequence_number)
        # Attribute 15: mesh_header_sequence_number (integer)
        result += self._encode_integer(self.mesh_header_sequence_number)
        # Attribute 16: routing_table (array)
        result += self._encode_array(self.routing_table)
        # Attribute 17: routing_table_update_time (octet-string)
        result += self._encode_octet_string(self.routing_table_update_time or b'')
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'WiSUNSetup':
        ...
        # Attribute 3: phy_operating_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_operating_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: network_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: pan_id
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['pan_id'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: routing_method
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['routing_method'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: routing_method_parameters
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['routing_method_parameters'] = cls._decode_array(value)
        # Attribute 8: phy_operating_mode_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['phy_operating_mode_list'] = cls._decode_array(value)
        # Attribute 9: channel_function
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['channel_function'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: channel_hopping_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['channel_hopping_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: unicast_dwell_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['unicast_dwell_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 12: broadcast_dwell_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['broadcast_dwell_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 13: broadcast_interval
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['broadcast_interval'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 14: broadcast_sequence_number
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['broadcast_sequence_number'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 15: mesh_header_sequence_number
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mesh_header_sequence_number'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 16: routing_table
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['routing_table'] = cls._decode_array(value)
        # Attribute 17: routing_table_update_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['routing_table_update_time'] = value
        return cls(**kwargs)
