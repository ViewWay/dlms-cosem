...
class IEC62055Attributes:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: sts_key_identification_no (integer)
        result += self._encode_integer(self.sts_key_identification_no)
        # Attribute 4: sts_key_revision_no (integer)
        result += self._encode_integer(self.sts_key_revision_no)
        # Attribute 5: sts_key_expiry_date (octet-string)
        result += self._encode_octet_string(self.sts_key_expiry_date or b'')
        # Attribute 6: sts_token_carrier_identification (octet-string)
        result += self._encode_octet_string(self.sts_token_carrier_identification or b'')
        # Attribute 7: sts_token_decoder_key_status (integer)
        result += self._encode_integer(self.sts_token_decoder_key_status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC62055Attributes':
        ...
        # Attribute 3: sts_key_identification_no
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sts_key_identification_no'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: sts_key_revision_no
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sts_key_revision_no'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: sts_key_expiry_date
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sts_key_expiry_date'] = value
        # Attribute 6: sts_token_carrier_identification
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sts_token_carrier_identification'] = value
        # Attribute 7: sts_token_decoder_key_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sts_token_decoder_key_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
