...
class SCHCLPWANSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: schc_rule_list (array)
        result += self._encode_array(self.schc_rule_list)
        # Attribute 4: schc_parameter_list (array)
        result += self._encode_array(self.schc_parameter_list)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SCHCLPWANSetup':
        ...
        # Attribute 3: schc_rule_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['schc_rule_list'] = cls._decode_array(value)
        # Attribute 4: schc_parameter_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['schc_parameter_list'] = cls._decode_array(value)
        return cls(**kwargs)
