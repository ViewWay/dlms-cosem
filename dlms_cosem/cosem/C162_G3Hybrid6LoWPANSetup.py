"""IC class 162 - G3Hybrid6LoWPANSetup.

G3-PLC Hybrid 6LoWPAN Adaptation Layer Setup - G3 hybrid 6LoWPAN configuration.

Blue Book: DLMS UA 1000-1 Ed. 16, class_id=162
"""
...
class G3Hybrid6LoWPANSetup:
    """COSEM IC G3Hybrid6LoWPANSetup (class_id=162).
    ...
    CLASS_ID: ClassVar[int] = enums.CosemInterface.G3_HYBRID_6LOWPAN_ADAPTATION_LAYER_SETUP
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: lowpan_enable (boolean)
        result += self._encode_boolean(self.lowpan_enable)
        # Attribute 4: lowpan_mtu (integer)
        result += self._encode_integer(self.lowpan_mtu)
        # Attribute 5: lowpan_fragmentation_timeout (integer)
        result += self._encode_integer(self.lowpan_fragmentation_timeout)
        # Attribute 6: lowpan_fragmentation_retries (integer)
        result += self._encode_integer(self.lowpan_fragmentation_retries)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'G3Hybrid6LoWPANSetup':
        ...
        # Attribute 3: lowpan_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lowpan_enable'] = bool(value[0]) if value else False
        # Attribute 4: lowpan_mtu
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lowpan_mtu'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: lowpan_fragmentation_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lowpan_fragmentation_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: lowpan_fragmentation_retries
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lowpan_fragmentation_retries'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
