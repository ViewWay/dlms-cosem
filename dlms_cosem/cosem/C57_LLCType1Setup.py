...
class LLCType1Setup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 4: llc_type_1_parameters (array)
        result += self._encode_array(self.llc_type_1_parameters)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'LLCType1Setup':
        ...
        # Attribute 4: llc_type_1_parameters
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['llc_type_1_parameters'] = cls._decode_array(value)
        return cls(**kwargs)
