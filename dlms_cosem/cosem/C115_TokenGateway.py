...
class TokenGateway:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: token (octet-string)
        result += self._encode_octet_string(self.token or b'')
        # Attribute 4: token_time (octet-string)
        result += self._encode_octet_string(self.token_time or b'')
        # Attribute 5: token_status (integer)
        result += self._encode_integer(self.token_status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'TokenGateway':
        ...
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['token'] = value
        # Attribute 4: token_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['token_time'] = value
        # Attribute 5: token_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['token_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
