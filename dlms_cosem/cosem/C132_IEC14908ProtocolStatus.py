...
class IEC14908ProtocolStatus:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: protocol_status (integer)
        result += self._encode_integer(self.protocol_status)
        # Attribute 4: connection_status (integer)
        result += self._encode_integer(self.connection_status)
        # Attribute 5: communication_statistics (array)
        result += self._encode_array(self.communication_statistics)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC14908ProtocolStatus':
        ...
        # Attribute 3: protocol_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['protocol_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: connection_status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['connection_status'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: communication_statistics
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['communication_statistics'] = cls._decode_array(value)
        return cls(**kwargs)
