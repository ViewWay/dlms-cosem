...
class SFSKMACCounters:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: tx_packet_count (integer)
        result += self._encode_integer(self.tx_packet_count)
        # Attribute 4: rx_packet_count (integer)
        result += self._encode_integer(self.rx_packet_count)
        # Attribute 5: crc_error_count (integer)
        result += self._encode_integer(self.crc_error_count)
        # Attribute 6: tx_time (integer)
        result += self._encode_integer(self.tx_time)
        # Attribute 7: rx_time (integer)
        result += self._encode_integer(self.rx_time)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SFSKMACCounters':
        ...
        # Attribute 3: tx_packet_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tx_packet_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: rx_packet_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rx_packet_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: crc_error_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['crc_error_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: tx_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tx_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: rx_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['rx_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
