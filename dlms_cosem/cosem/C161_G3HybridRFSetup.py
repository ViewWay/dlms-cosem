"""IC class 161 - G3HybridRFSetup.

G3-PLC Hybrid RF MAC Setup - G3 hybrid RF MAC configuration.

Blue Book: DLMS UA 1000-1 Ed. 16, class_id=161
"""
...
class G3HybridRFSetup:
    """COSEM IC G3HybridRFSetup (class_id=161).
    ...
    CLASS_ID: ClassVar[int] = enums.CosemInterface.G3_HYBRID_RF_MAC_SETUP
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mac_address (octet-string)
        result += self._encode_octet_string(self.mac_address or b'')
        # Attribute 4: mac_frame_counter (integer)
        result += self._encode_integer(self.mac_frame_counter)
        # Attribute 5: mac_key (octet-string)
        result += self._encode_octet_string(self.mac_key or b'')
        # Attribute 6: mac_switch (boolean)
        result += self._encode_boolean(self.mac_switch)
        # Attribute 7: mac_security_enabled (boolean)
        result += self._encode_boolean(self.mac_security_enabled)
        # Attribute 8: mac_security_level (integer)
        result += self._encode_integer(self.mac_security_level)
        # Attribute 9: mac_routing_mode (integer)
        result += self._encode_integer(self.mac_routing_mode)
        # Attribute 10: mac_tx_power (integer)
        result += self._encode_integer(self.mac_tx_power)
        # Attribute 11: mac_tx_retries (integer)
        result += self._encode_integer(self.mac_tx_retries)
        # Attribute 12: mac_tx_ack_timeout (integer)
        result += self._encode_integer(self.mac_tx_ack_timeout)
        # Attribute 13: mac_tx_data_rate (integer)
        result += self._encode_integer(self.mac_tx_data_rate)
        # Attribute 14: mac_tx_power_control (boolean)
        result += self._encode_boolean(self.mac_tx_power_control)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'G3HybridRFSetup':
        ...
        # Attribute 3: mac_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_address'] = value
        # Attribute 4: mac_frame_counter
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_frame_counter'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: mac_key
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_key'] = value
        # Attribute 6: mac_switch
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_switch'] = bool(value[0]) if value else False
        # Attribute 7: mac_security_enabled
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_security_enabled'] = bool(value[0]) if value else False
        # Attribute 8: mac_security_level
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_security_level'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 9: mac_routing_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_routing_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 10: mac_tx_power
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_power'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 11: mac_tx_retries
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_retries'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 12: mac_tx_ack_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_ack_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 13: mac_tx_data_rate
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_data_rate'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 14: mac_tx_power_control
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_power_control'] = bool(value[0]) if value else False
        return cls(**kwargs)
