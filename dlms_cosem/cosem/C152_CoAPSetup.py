...
class CoAPSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: coap_server_address (string)
        result += self._encode_string(self.coap_server_address or '')
        # Attribute 4: coap_server_port (integer)
        result += self._encode_integer(self.coap_server_port)
        # Attribute 5: response_timeout (integer)
        result += self._encode_integer(self.response_timeout)
        # Attribute 6: max_retransmit (integer)
        result += self._encode_integer(self.max_retransmit)
        # Attribute 7: ack_timeout (integer)
        result += self._encode_integer(self.ack_timeout)
        # Attribute 8: ack_random_factor (integer)
        result += self._encode_integer(self.ack_random_factor)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'CoAPSetup':
        ...
        # Attribute 3: coap_server_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['coap_server_address'] = value.decode('utf-8') if value else ''
        # Attribute 4: coap_server_port
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['coap_server_port'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: response_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['response_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: max_retransmit
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['max_retransmit'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: ack_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['ack_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: ack_random_factor
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['ack_random_factor'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
