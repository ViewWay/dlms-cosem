...
class IPv6Setup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: address_configuration_mode (integer)
        result += self._encode_integer(self.address_configuration_mode)
        # Attribute 4: link_local_address (octet-string)
        result += self._encode_octet_string(self.link_local_address or b'')
        # Attribute 5: address_list (array)
        result += self._encode_array(self.address_list)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IPv6Setup':
        ...
        # Attribute 3: address_configuration_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['address_configuration_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: link_local_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['link_local_address'] = value
        # Attribute 5: address_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['address_list'] = cls._decode_array(value)
        return cls(**kwargs)
