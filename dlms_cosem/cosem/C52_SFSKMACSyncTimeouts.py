...
class SFSKMACSyncTimeouts:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: time_outs (array)
        result += self._encode_array(self.time_outs)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SFSKMACSyncTimeouts':
        ...
        # Attribute 3: time_outs
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['time_outs'] = cls._decode_array(value)
        return cls(**kwargs)
