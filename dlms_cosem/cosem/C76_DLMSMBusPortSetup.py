...
class DLMSMBusPortSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: m_bus_port_reference (string)
        result += self._encode_string(self.m_bus_port_reference or '')
        # Attribute 4: listen_port (integer)
        result += self._encode_integer(self.listen_port)
        # Attribute 5: slave_devices (array)
        result += self._encode_array(self.slave_devices)
        # Attribute 6: client_active (boolean)
        result += self._encode_boolean(self.client_active)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'DLMSMBusPortSetup':
        ...
        # Attribute 3: m_bus_port_reference
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['m_bus_port_reference'] = value.decode('utf-8') if value else ''
        # Attribute 4: listen_port
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['listen_port'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: slave_devices
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['slave_devices'] = cls._decode_array(value)
        # Attribute 6: client_active
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['client_active'] = bool(value[0]) if value else False
        return cls(**kwargs)
