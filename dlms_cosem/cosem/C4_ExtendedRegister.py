...
class ExtendedRegister:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: value
        result += self._encode_octet_string(self._any_to_bytes(self.value))
        # Attribute 4: capture_time (datetime)
        result += self._encode_datetime(self.capture_time)
        # Attribute 5: scaler (integer)
        result += self._encode_integer(self.scaler)
        # Attribute 6: unit (integer)
        result += self._encode_integer(self.unit)
        # Attribute 7: status (integer)
        result += self._encode_integer(self.status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ExtendedRegister':
