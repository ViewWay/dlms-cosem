...
class ZigbeeTunnelSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: tunnel_address (octet-string)
        result += self._encode_octet_string(self.tunnel_address or b'')
        # Attribute 4: tunnel_port (integer)
        result += self._encode_integer(self.tunnel_port)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ZigbeeTunnelSetup':
        ...
        # Attribute 3: tunnel_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tunnel_address'] = value
        # Attribute 4: tunnel_port
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['tunnel_port'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
