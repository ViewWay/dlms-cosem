...
class ParameterMonitor:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: parameter_list (array)
        result += self._encode_array(self.parameter_list)
        # Attribute 4: parameter_value (array)
        result += self._encode_array(self.parameter_value)
        # Attribute 5: capture_time (octet-string)
        result += self._encode_octet_string(self.capture_time or b'')
        # Attribute 6: changed_parameter (structure)
        result += self._encode_structure(self.changed_parameter)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ParameterMonitor':
        ...
        # Attribute 3: parameter_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parameter_list'] = cls._decode_array(value)
        # Attribute 4: parameter_value
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['parameter_value'] = cls._decode_array(value)
        # Attribute 5: capture_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['capture_time'] = value
        # Attribute 6: changed_parameter
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['changed_parameter'] = cls._decode_structure(value)
        return cls(**kwargs)
