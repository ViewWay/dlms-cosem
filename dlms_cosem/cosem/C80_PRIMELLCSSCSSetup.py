...
class PRIMELLCSSCSSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: sscs_type (integer)
        result += self._encode_integer(self.sscs_type)
        # Attribute 4: sscs_enable (boolean)
        result += self._encode_boolean(self.sscs_enable)
        # Attribute 5: sscs_response_time (integer)
        result += self._encode_integer(self.sscs_response_time)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'PRIMELLCSSCSSetup':
        ...
        # Attribute 3: sscs_type
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sscs_type'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: sscs_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sscs_enable'] = bool(value[0]) if value else False
        # Attribute 5: sscs_response_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['sscs_response_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
