...
class DisconnectControl:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: output_state (integer)
        result += self._encode_integer(self.output_state)
        # Attribute 4: control_state (integer)
        result += self._encode_integer(self.control_state)
        # Attribute 5: control_mode (integer)
        result += self._encode_integer(self.control_mode)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'DisconnectControl':
        ...
        # Attribute 3: output_state
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['output_state'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: control_state
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['control_state'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: control_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['control_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
