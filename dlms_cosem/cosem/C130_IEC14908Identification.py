...
class IEC14908Identification:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: domain_address (integer)
        result += self._encode_integer(self.domain_address)
        # Attribute 4: subnet_address (integer)
        result += self._encode_integer(self.subnet_address)
        # Attribute 5: node_address (integer)
        result += self._encode_integer(self.node_address)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC14908Identification':
        ...
        # Attribute 3: domain_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['domain_address'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: subnet_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['subnet_address'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: node_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['node_address'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
