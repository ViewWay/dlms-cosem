...
class ArrayManager:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: data_array (array)
        result += self._encode_array(self.data_array)
        # Attribute 4: array_size (integer)
        result += self._encode_integer(self.array_size)
        # Attribute 5: element_type (integer)
        result += self._encode_integer(self.element_type)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ArrayManager':
        ...
        # Attribute 3: data_array
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['data_array'] = cls._decode_array(value)
        # Attribute 4: array_size
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['array_size'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: element_type
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['element_type'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
