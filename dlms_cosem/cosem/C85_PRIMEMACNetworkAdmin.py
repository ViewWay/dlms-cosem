...
class PRIMEMACNetworkAdmin:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: network_id (integer)
        result += self._encode_integer(self.network_id)
        # Attribute 4: network_role (integer)
        result += self._encode_integer(self.network_role)
        # Attribute 5: network_state (integer)
        result += self._encode_integer(self.network_state)
        # Attribute 6: network_device_list (array)
        result += self._encode_array(self.network_device_list)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'PRIMEMACNetworkAdmin':
        ...
        # Attribute 3: network_id
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_id'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: network_role
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_role'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: network_state
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_state'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: network_device_list
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['network_device_list'] = cls._decode_array(value)
        return cls(**kwargs)
