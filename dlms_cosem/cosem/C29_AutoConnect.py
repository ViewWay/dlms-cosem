...
class AutoConnect:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mode (integer)
        result += self._encode_integer(self.mode)
        # Attribute 4: repetitions (integer)
        result += self._encode_integer(self.repetitions)
        # Attribute 5: repetition_delay (integer)
        result += self._encode_integer(self.repetition_delay)
        # Attribute 6: calling_window (array)
        result += self._encode_array(self.calling_window)
        # Attribute 7: allowed_destinations (array)
        result += self._encode_array(self.allowed_destinations)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'AutoConnect':
        ...
        # Attribute 4: repetitions
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['repetitions'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: repetition_delay
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['repetition_delay'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: calling_window
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['calling_window'] = cls._decode_array(value)
        # Attribute 7: allowed_destinations
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['allowed_destinations'] = cls._decode_array(value)
        return cls(**kwargs)
