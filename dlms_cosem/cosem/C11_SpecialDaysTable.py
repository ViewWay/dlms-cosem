...
class SpecialDayTable:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: special_day_table (array)
        result += self._encode_array(self.special_day_table)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'SpecialDayTable':
        ...
        # Attribute 3: special_day_table
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['special_day_table'] = cls._decode_array(value)
        return cls(**kwargs)
