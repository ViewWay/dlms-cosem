...
class PRIMEMACCounters:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mac_tx_total (integer)
        result += self._encode_integer(self.mac_tx_total)
        # Attribute 4: mac_rx_total (integer)
        result += self._encode_integer(self.mac_rx_total)
        # Attribute 5: mac_tx_error (integer)
        result += self._encode_integer(self.mac_tx_error)
        # Attribute 6: mac_rx_error (integer)
        result += self._encode_integer(self.mac_rx_error)
        # Attribute 7: mac_tx_dropped (integer)
        result += self._encode_integer(self.mac_tx_dropped)
        # Attribute 8: mac_rx_dropped (integer)
        result += self._encode_integer(self.mac_rx_dropped)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'PRIMEMACCounters':
        ...
        # Attribute 3: mac_tx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: mac_rx_total
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_total'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: mac_tx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: mac_rx_error
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_error'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: mac_tx_dropped
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_dropped'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: mac_rx_dropped
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_dropped'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
