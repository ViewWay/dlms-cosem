...
class HSPLCHDLCSSASSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: hdlc_ssas_enable (boolean)
        result += self._encode_boolean(self.hdlc_ssas_enable)
        # Attribute 4: hdlc_ssas_mtu (integer)
        result += self._encode_integer(self.hdlc_ssas_mtu)
        # Attribute 5: hdlc_ssas_fragmentation_timeout (integer)
        result += self._encode_integer(self.hdlc_ssas_fragmentation_timeout)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'HSPLCHDLCSSASSetup':
        ...
        # Attribute 3: hdlc_ssas_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['hdlc_ssas_enable'] = bool(value[0]) if value else False
        # Attribute 4: hdlc_ssas_mtu
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['hdlc_ssas_mtu'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: hdlc_ssas_fragmentation_timeout
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['hdlc_ssas_fragmentation_timeout'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
