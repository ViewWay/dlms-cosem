...
class IEC61334LLCSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: llc_type_1_enable (boolean)
        result += self._encode_boolean(self.llc_type_1_enable)
        # Attribute 4: llc_type_2_enable (boolean)
        result += self._encode_boolean(self.llc_type_2_enable)
        # Attribute 5: llc_type_3_enable (boolean)
        result += self._encode_boolean(self.llc_type_3_enable)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'IEC61334LLCSetup':
        ...
        # Attribute 3: llc_type_1_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['llc_type_1_enable'] = bool(value[0]) if value else False
        # Attribute 4: llc_type_2_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['llc_type_2_enable'] = bool(value[0]) if value else False
        # Attribute 5: llc_type_3_enable
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
