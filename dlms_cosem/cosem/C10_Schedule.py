...
class ActionSchedule:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: activate_passive_time (octet-string)
        result += self._encode_octet_string(self.activate_passive_time or b'')
        # Attribute 4: action_schedule (array)
        result += self._encode_array(self.action_schedule)
        # Attribute 5: executions (array)
        result += self._encode_array(self.executions)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ActionSchedule':
        ...
        # Attribute 3: activate_passive_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['activate_passive_time'] = value
        # Attribute 4: action_schedule
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['action_schedule'] = cls._decode_array(value)
        # Attribute 5: executions
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['executions'] = cls._decode_array(value)
        return cls(**kwargs)
