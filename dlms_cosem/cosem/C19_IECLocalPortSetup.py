...
class LocalPortSetup:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: default_mode (integer)
        result += self._encode_integer(self.default_mode)
        # Attribute 4: default_baudrate (integer)
        result += self._encode_integer(self.default_baudrate)
        # Attribute 5: proposed_baudrate (integer)
        result += self._encode_integer(self.proposed_baudrate)
        # Attribute 6: response_time (integer)
        result += self._encode_integer(self.response_time)
        # Attribute 7: device_address (octet-string)
        result += self._encode_octet_string(self.device_address or b'')
        # Attribute 8: password (octet-string)
        result += self._encode_octet_string(self.password or b'')
        # Attribute 9: repeat_count (integer)
        result += self._encode_integer(self.repeat_count)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'LocalPortSetup':
        ...
        # Attribute 3: default_mode
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['default_mode'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: default_baudrate
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['default_baudrate'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: proposed_baudrate
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['proposed_baudrate'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: response_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['response_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: device_address
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['device_address'] = value
        # Attribute 8: password
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['password'] = value
        # Attribute 9: repeat_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['repeat_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
