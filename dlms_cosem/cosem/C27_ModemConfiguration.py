...
class ModemConfiguration:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: connection_settings (octet-string)
        result += self._encode_octet_string(self.connection_settings or b'')
        # Attribute 4: status (integer)
        result += self._encode_integer(self.status)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ModemConfiguration':
        ...
        # Attribute 3: connection_settings
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['connection_settings'] = value
        # Attribute 4: status
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
