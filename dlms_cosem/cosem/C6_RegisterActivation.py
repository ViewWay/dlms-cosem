...
class RegisterActivation:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: register_assignment (array)
        result += self._encode_array(self.register_assignment)
        # Attribute 4: activation_mask (integer)
        result += self._encode_integer(self.activation_mask)
        # Attribute 5: active_mask (integer)
        result += self._encode_integer(self.active_mask)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'RegisterActivation':
        ...
        # Attribute 3: register_assignment
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['register_assignment'] = cls._decode_array(value)
        # Attribute 4: activation_mask
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['activation_mask'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: active_mask
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['active_mask'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
