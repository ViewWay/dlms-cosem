...
class ZigbeeSASAPSFragmentation:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: fragmentation_enabled (boolean)
        result += self._encode_boolean(self.fragmentation_enabled)
        # Attribute 4: window_size (integer)
        result += self._encode_integer(self.window_size)
        # Attribute 5: inter_frame_delay (integer)
        result += self._encode_integer(self.inter_frame_delay)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'ZigbeeSASAPSFragmentation':
        ...
        # Attribute 3: fragmentation_enabled
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['fragmentation_enabled'] = bool(value[0]) if value else False
        # Attribute 4: window_size
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['window_size'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: inter_frame_delay
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['inter_frame_delay'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
