...
class LoRaWANDiagnostic:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 7: lorawan_rssi (integer)
        result += self._encode_integer(self.lorawan_rssi)
        # Attribute 8: lorawan_snr (integer)
        result += self._encode_integer(self.lorawan_snr)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'LoRaWANDiagnostic':
        ...
        # Attribute 7: lorawan_rssi
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_rssi'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 8: lorawan_snr
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['lorawan_snr'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
