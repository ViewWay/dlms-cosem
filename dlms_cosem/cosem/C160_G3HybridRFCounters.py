...
class G3HybridRFCounters:
    ...
    def to_bytes(self) -> bytes:
        ...
        # Attribute 3: mac_tx_packet_count (integer)
        result += self._encode_integer(self.mac_tx_packet_count)
        # Attribute 4: mac_rx_packet_count (integer)
        result += self._encode_integer(self.mac_rx_packet_count)
        # Attribute 5: mac_crc_error_count (integer)
        result += self._encode_integer(self.mac_crc_error_count)
        # Attribute 6: mac_tx_time (integer)
        result += self._encode_integer(self.mac_tx_time)
        # Attribute 7: mac_rx_time (integer)
        result += self._encode_integer(self.mac_rx_time)
        return bytes(result)
    @classmethod
    def from_bytes(cls, data: bytes) -> 'G3HybridRFCounters':
        ...
        # Attribute 3: mac_tx_packet_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_packet_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 4: mac_rx_packet_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_packet_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 5: mac_crc_error_count
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_crc_error_count'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 6: mac_tx_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_tx_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        # Attribute 7: mac_rx_time
        if offset < len(data):
            tag, value, offset = cls._decode_tlv(data, offset)
            kwargs['mac_rx_time'] = int.from_bytes(value, 'big', signed=True) if value else 0
        return cls(**kwargs)
